...
if __name__ == "__main__":
    ...
    _means, matrix = cal_fisher(text_cnn, comp_train_loader)

    # comp->rec
    for epoch in range(10):
        ewc_train(text_cnn, optimizer, _means, matrix, rec_train_loader, lambda1, lambda2)
        temp = valid(text_cnn, comp_test_loader)
        print('task 1 accuracy: {} % '.format(100 * temp))
        acc_comp.append(temp)
        temp = valid(text_cnn, rec_test_loader)
        acc_rec.append(temp)
        print('task 2 accuracy: {} % '.format(100 * temp))

    # comp->rec->sci
    _means, matrix = cal_fisher(text_cnn, rec_train_loader)
    for epoch in range(10):
        ewc_train(text_cnn, optimizer, _means, matrix, sci_train_loader, lambda1, lambda2)
        temp = valid(text_cnn, comp_test_loader)
        acc_comp.append(temp)
        print('task 1 accuracy: {} % '.format(100 * temp))
        temp = valid(text_cnn, rec_test_loader)
        ...
        acc_sci.append(temp)
        print('task 3 accuracy: {} % '.format(100 * temp))

    _means, matrix = cal_fisher(text_cnn, sci_train_loader)
    for epoch in range(10):
        ewc_train(text_cnn, optimizer, _means, matrix, talk_train_loader, lambda1, lambda2)
        temp = valid(text_cnn, comp_test_loader)
        acc_comp.append(temp)
        print('task 1 accuracy: {} % '.format(100 * temp))
        temp = valid(text_cnn, rec_test_loader)
        ...
        print('task 4 accuracy: {} % '.format(100 * temp))
    ...
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.title('outputs using EWC,lambda1=%d,lambda2=%d' %(lambda1,lambda2))
    #plt.title('Multitask training with EWC')
    plt.legend()
    plt.show()
